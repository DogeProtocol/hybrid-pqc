...
int test_hybrid_dilithium_perf(int count);
...
int test_sphincs() {
	...
	int r1 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_keypair(pk, sk);
	if (r1 != 0) {
		printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_keypair failed %d", (int)r1);
		return r1;
	}
	...
	int r3 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign(sig, &sigLen, msg1, 32, sk);
	if (r3 != 0) {
		printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign failed %d", (int)r3);
		return r3;
	}

	unsigned long long msgLen = 0;
	int r4 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open(msg2, &msgLen, sig, sigLen, pk);
	if (r4 != 0) {
		printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open failed %d", (int)r4);
		return r4;
	}
	if (msgLen != 32) {
		printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open msg check failed %d", (int)msgLen);
		return -5;
	}
	...
	int r5 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_seed_keypair(pk2, sk2, seed1);
	if (r5 != 0) {
		printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_seed_keypair failed %d", (int)r5);
		return r5;
	}

	for (int j = 0; j < 32; j++) {
		unsigned char pk3[32];
		unsigned char sk3[64];
		int r6 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_seed_keypair(pk3, sk3, seed1);
		if (r6 != 0) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_seed_keypair failed %d", (int)r6);
			return r6;
		}
		for (int i = 0; i < 32; i++) {
			if (pk2[i] != pk3[i]) {
		...
		int r = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign(sig, &sigLen, msg1, 32, sk3);
		if (r != 0) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign failed %d", (int)r);
			return r;
		}

		unsigned long long msgLen = 0;
		r = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open(msg2, &msgLen, sig, sigLen, pk3);
		if (r != 0) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open failed %d", (int)r);
			return r;
		}
		if (msgLen != 32) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open msg check failed %d", (int)msgLen);
	...
	for (int i = 0; i < 1000; i++) {
		r3 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign(sig, &sigLen, msg1, 32, sk);
		if (r3 != 0) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign failed %d", (int)r3);
			return r3;
		}
	...
	for (int i = 0; i < 10000; i++) {
		r4 = PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open(msg2, &msgLen, sig, sigLen, pk);
		if (r4 != 0) {
			printf("\n PQCLEAN_SPHINCSSHAKE128FSIMPLE_CLEAN_crypto_sign_open failed %d", (int)r4);
			return r4;
		}
...
int main(int argc, char* argv[]) {
	int rdh = test_hybrid_dilithium();
	if (rdh != 0) {
		return rdh;
	}

	rdh = test_hybrid_dilithium_deterministic();
	if (rdh != 0) {
		return rdh;
	}

	int rd = test_dilithium();
	if (rd != 0) {
		return rd;
	}
	...
	r3 = test_hybrid_falcon_deterministic();
	if (r3 != 0) {
		return r3;
	}
